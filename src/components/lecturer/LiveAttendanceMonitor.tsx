...
export const LiveAttendanceMonitor: React.FC<LiveAttendanceMonitorProps> = ({
  ...
  const handleStudentCheckIn = useCallback(
    (student: Student) => {
      setStudents((prev) => {
  ...
  }, [classId, handleStudentCheckIn]);

  return (
    <div className="space-y-4">
      <div className="rounded-lg bg-white p-6 shadow dark:bg-gray-800">
        <div className="mb-6 flex items-center justify-between">
          <h3 className="text-xl font-bold text-gray-900 dark:text-white">Live Attendance</h3>
          <button
            ...
            <tbody className="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-900">
              {students.map((student) => (
                <tr key={student.id}>
                  ...
                  <td className="whitespace-nowrap px-6 py-4">
                    <span
                      className={`inline-flex rounded-full px-2 py-1 text-xs font-semibold ${student.status === 'present' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : ''} ${student.status === 'late' ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200' : ''} ${student.status === 'absent' ? 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200' : ''} ${student.status === 'pending' ? 'bg-gray-100 text-gray-800 dark:bg-gray-900 dark:text-gray-200' : ''} `}
                    >
